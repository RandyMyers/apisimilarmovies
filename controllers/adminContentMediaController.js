...
const { uploadEditorContentBuffer } = require('../services/adCloudinaryUploadService');

function pickUploadedFile(req) {
  const files = req.files;
  if (!files || typeof files !== 'object') return null;
  const candidates = [files.image, files.file, files.upload];
...
exports.uploadContentImage = asyncHandler(async (req, res) => {
  ...
  try {
    const data = await uploadEditorContentBuffer(file.data, {
      mimetype: file.mimetype,
    });
    return res.status(201).json({ success: true, url: data.url, data });
  } catch (err) {
    console.error('[adminContentMedia] uploadContentImage failed:', err?.message || err);
