...
function toSafeUser(user) {
  return {
    ...
    settings: {
      emailNotifs: user?.settings?.emailNotifs !== false,
      contentTips: user?.settings?.contentTips !== false,
      publicProfile: user?.settings?.publicProfile === true,
    },
    siteKey: user.siteKey || 'default',
  };
}
...
exports.login = async (req, res) => {
  try {
    ...
    const password = String(req.body?.password || '');
    if (!email || !password) return res.status(400).json({ error: 'email and password are required' });
    const user = await User.findOne({ siteKey, email, deletedAt: null });
